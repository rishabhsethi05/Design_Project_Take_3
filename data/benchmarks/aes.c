/*
 * BEEBS AES-128 Benchmark (5x Scaled Workload)
 * Original code based on SNU-RT / BEEBS AES encryption benchmark.
 * Adapted for standalone MAPI-PRO simulation without external headers.
 */

typedef unsigned char uint8_t;

// AES-128 Constants
#define AES_BLOCK_SIZE 16
#define AES_KEY_SIZE 16
#define AES_EXP_KEY_SIZE 176

// Fixed S-Box for AES byte substitution
static const uint8_t sbox[256] = {
    
};

// Round constants for key expansion
static const uint8_t Rcon[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

// Volatile state arrays stored in system memory
uint8_t state[16];
uint8_t key[16];
uint8_t RoundKey[176];

static uint8_t xtime(uint8_t x)
{
    return ((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static void KeyExpansion(void)
{
    int i, j;
    uint8_t tempa[4];

    for (i = 0; i < 4; ++i) {
        RoundKey[(i * 4) + 0] = key[(i * 4) + 0];
        RoundKey[(i * 4) + 1] = key[(i * 4) + 1];
        RoundKey[(i * 4) + 2] = key[(i * 4) + 2];
        RoundKey[(i * 4) + 3] = key[(i * 4) + 3];
    }

    for (i = 4; i < 44; ++i) {
        j = (i - 1) * 4;
        tempa[0] = RoundKey[j + 0];
        tempa[1] = RoundKey[j + 1];
        tempa[2] = RoundKey[j + 2];
        tempa[3] = RoundKey[j + 3];

        if (i % 4 == 0) {
            uint8_t k = tempa[0];
            tempa[0] = sbox[tempa[1]] ^ Rcon[i / 4];
            tempa[1] = sbox[tempa[2]];
            tempa[2] = sbox[tempa[3]];
            tempa[3] = sbox[k];
        }

        j = i * 4;
        RoundKey[j + 0] = RoundKey[(i - 4) * 4 + 0] ^ tempa[0];
        RoundKey[j + 1] = RoundKey[(i - 4) * 4 + 1] ^ tempa[1];
        RoundKey[j + 2] = RoundKey[(i - 4) * 4 + 2] ^ tempa[2];
        RoundKey[j + 3] = RoundKey[(i - 4) * 4 + 3] ^ tempa[3];
    }
}

static void AddRoundKey(int round)
{
    int i;
    for (i = 0; i < 16; ++i) {
        state[i] ^= RoundKey[(round * 16) + i];
    }
}

static void SubBytes(void)
{
    int i;
    for (i = 0; i < 16; ++i) {
        state[i] = sbox[state[i]];
    }
}

static void ShiftRows(void)
{
    uint8_t temp;

    // Row 1
    temp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = temp;

    // Row 2
    temp = state[2];
    state[2] = state[10];
    state[10] = temp;
    temp = state[6];
    state[6] = state[14];
    state[14] = temp;

    // Row 3
    temp = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = state[3];
    state[3] = temp;
}

static void MixColumns(void)
{
    int i;
    uint8_t a, b, c, d, e;
    for (i = 0; i < 4; ++i) {
        a = state[i * 4 + 0];
        b = state[i * 4 + 1];
        c = state[i * 4 + 2];
        d = state[i * 4 + 3];
        e = a ^ b ^ c ^ d;

        state[i * 4 + 0] ^= e ^ xtime(a ^ b);
        state[i * 4 + 1] ^= e ^ xtime(b ^ c);
        state[i * 4 + 2] ^= e ^ xtime(c ^ d);
        state[i * 4 + 3] ^= e ^ xtime(d ^ a);
    }
}

static void Cipher(void)
{
    int round = 0;

    AddRoundKey(0);

    for (round = 1; round < 10; ++round) {
        SubBytes();
        ShiftRows();
        MixColumns();
        AddRoundKey(round);
    }

    SubBytes();
    ShiftRows();
    AddRoundKey(10);
}

void initialise_benchmark(void)
{
    int i;
    // Standard test vector initialization
    for (i = 0; i < 16; ++i) {
        state[i] = (uint8_t)(i * 17 + 5);
        key[i] = (uint8_t)(i * 31 + 3);
    }
    KeyExpansion();
}

int benchmark(void)
{
    int pass;

    /*
     * Native 5x Workload scaling.
     * Executes 5 consecutive AES-128 block encryptions over the state array.
     */
    for (pass = 0; pass < 5; pass++) {
        Cipher();
    }

    return 0;
}

int main(void)
{
    initialise_benchmark();
    benchmark();
    return 0;
}